...
export default function PaymentsPage() {
  ...
  if (!authorized)
    return (
      <main className="min-h-screen pt-14 flex items-center justify-center">
        <p className="text-gray-500">Please login to make payments</p>
      </main>
    );
  ...
  const handlePay = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orderData = await apiAuthPost<{
        success: boolean;
        data: {
          appointmentId: string;
          order: { id: string; amount: number; currency: string };
        };
      }>("/api/payments/create-order", { appointmentId });
      ...
      const razorpay = new (window.Razorpay as any)({
        ...
        handler: async (res: any) => {
          try {
            const verify = await apiAuthPost<{ success: boolean; message: string }>(
              "/api/payments/verify",
              {
  ...
  return (
    <main className="min-h-screen pt-14 px-4 py-8 max-w-md mx-auto">
      ...
      <form
        onSubmit={handlePay}
        className="mt-5 space-y-4 rounded-xl border border-white/10 bg-white/5 p-5"
      >
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-black/20 py-2 pl-9 pr-3 text-white placeholder-gray-500 outline-none focus:border-emerald-500"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 rounded-lg bg-emerald-500 py-2.5 text-sm font-medium text-white hover:bg-emerald-600 disabled:opacity-60"
        >
